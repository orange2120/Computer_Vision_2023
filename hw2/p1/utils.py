...
def get_tiny_images(img_paths):
    ...
    for path in tqdm(img_paths):
        img = Image.open(path)
        img = np.asarray(img.resize((IMG_SZ, IMG_SZ)), dtype=np.float32)
        img = img.flatten()
        img = img - img.mean()
        # normalize by infinity L2-norm
        img = img / np.linalg.norm(img, ord=2)
        # normalize by infinity norm
        # img = img/np.linalg.norm(img, ord=np.inf)
...
def build_vocabulary(img_paths, vocab_size=400):
    ...
    for path in tqdm(img_paths):
        ...
        _, descriptors = dsift(img, step=[STEPS,STEPS], fast=True)
...
def get_bags_of_sifts(img_paths, vocab):
    ...
    for path in tqdm(img_paths):
        img = np.asarray(Image.open(path).convert('L')) # read as grayscale
        _, descriptors = dsift(img, step=[STEPS,STEPS], fast=True)
        dist = cdist(vocab, descriptors, metric="euclidean")
        
        hist, _ = np.histogram(np.argmin(dist, axis=0), bins=vocab.shape[0])
        hist_norm = hist/np.sum(hist) # normalization
...
def nearest_neighbor_classify(train_img_feats, train_labels, test_img_feats):
    ...
    # foreach distance
    for d in dist:
        index = np.argsort(d)[:THRESHOLD]
        labels = [train_labels[i] for i in index]
        label_ids = [CAT2ID[i] for i in labels] # convert to label ID

        final_id = np.argmax(np.bincount(label_ids))
