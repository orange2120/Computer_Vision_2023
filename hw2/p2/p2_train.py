...
def plot_learning_curve(logfile_dir, result_lists):
    ...
    plt.plot(result_lists['train_acc'], color='blue', label='Training acc.')
    plt.plot(result_lists['val_acc'], color='red', label='Validation acc.')
    ...
    plt.plot(result_lists['train_loss'], color='blue', label='Training loss')
    plt.plot(result_lists['val_loss'], color='red', label='Validation loss')
    ...
    plt1.savefig(os.path.join(logfile_dir, 'acc.svg'), format='svg')
    plt2.savefig(os.path.join(logfile_dir, 'loss.svg'), format='svg')
    ...
    # plt2 = plt.figure(2)
    # plt.plot(result_lists['val_acc'], color='blue', label='Validation acc.')
    ...
    # plt4 = plt.figure(4)
    # plt.plot(result_lists['val_loss'], color='blue', label='Validation loss')
    # plt.title('Validation loss')
    # plt.xlabel('Epochs')
    # plt.ylabel('Loss')

    # plt1.savefig(os.path.join(logfile_dir, 'train_acc.svg'))
    # plt2.savefig(os.path.join(logfile_dir, 'val_acc.svg'))
    # plt3.savefig(os.path.join(logfile_dir, 'train_loss.svg'))
    # plt4.savefig(os.path.join(logfile_dir, 'val_loss.svg'))
    ...
    for epoch in range(cfg.epochs):
        ...
        with torch.no_grad():
            ...
            for data in val_loader:
                ...
                val_correct += torch.sum(torch.argmax(target, dim=1) == labels)
                val_loss += loss.item()
