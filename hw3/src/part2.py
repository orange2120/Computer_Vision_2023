...
def planarAR(REF_IMAGE_PATH, VIDEO_PATH):
    ...
    while (video.isOpened()):
        ...
        if ret:  ## check whethere the frame is legal, i.e., there still exists a frame
            ...
            corn = corners[0][0].astype(int) # extract 1st marker
            H = solve_homography(ref_corns, corn)

            # TODO: 3.apply backward warp
            # function call to warping()
            cmin = corn.min(axis=0)
            cmax = corn.max(axis=0)
            frame = warping(ref_image, frame, H, cmin[1], cmax[1], cmin[0], cmax[0], direction='b')
